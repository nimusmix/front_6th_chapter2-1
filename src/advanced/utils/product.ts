...
export const getProductById = (productList: Product[], id: string): Product | undefined => {
  return productList.find((p) => p.id === id);
};

export const getLightningSaleProduct = (productList: Product[]): Product | null => {
...
export const findRecommendedProduct = (
  productList: Product[],
  excludeId: string,
): Product | undefined => {
...
export const getTotalStock = (productList: Product[]): number => {
  return productList.reduce((sum, p) => sum + p.quantity, 0);
};

export const getProductNameWithBadge = (product: Product): string => {
  let prefix = '';
...
export const getProductPriceHTML = (product: Product): string => {
